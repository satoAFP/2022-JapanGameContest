...
public class InputColor_Script : Base_Color_Script
{
    ...
    public void Start()
    {
        if(energization == true) GetComponent<Renderer>().material.color = new Color32((byte)color[COLOR_RED], (byte)color[COLOR_GREEN], (byte)color[COLOR_BLUE], 255);
    }
    ...
    public void OnCollisionStay(Collision collision)
    {
        if(collision.gameObject.tag=="ColorOutput")
        {
            if (energization == true && collision.gameObject.GetComponent<Base_Enegization>().GetEnergization() == false)
            {
                ...
                collision.gameObject.GetComponent<Renderer>().material.color = new Color32((byte)color[COLOR_RED], (byte)color[COLOR_GREEN], (byte)color[COLOR_BLUE], 255);
